...
from src.utils import get_bn_counts, get_cpt_index, get_cpt_shape, get_min_max_bns, get_tabular_cpt, learn_bn_params, vac_cn
...
class CN:
    def __init__(self, bn_min_max: tuple = None, cn:gum.CredalNet=None):
        ...
        if bn_min_max:
            ...
            raise RuntimeError("Please provide either a pair of BNs or a gum.CredalNet.")
...
class PriorCN(CN):
    ...
    def is_vacuous_any(self):
        ...
        for var in self.names():
            if self.is_vacuous_cpt(var): return True
        return False
    
    def is_vacuous_all(self):

        for var in self.names():
            if not self.is_vacuous_cpt(var): return False
        return True
...
class Client:
    ...
    def mosaic_cn_cpt(self, var:str):
        ...
        parents_conf = [dict(index_df.iloc[i]) for i in range(len(index_df))] if len(self.bn.parents(var)) != 0 else [None]
        for parents in parents_conf:

            parents_idx = get_cpt_index(self.bn, var, parents)

            # ... get the prior, ...
            prior_min, prior_max = prior_cpt_min[parents_idx, :], prior_cpt_max[parents_idx, :]
